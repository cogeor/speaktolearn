...
| 5 | Mobile Integration | Weeks 17-20 | Flutter FFI, ONNX Runtime, UI components | End-to-end scoring < 500ms on mid-tier device |
...
- ONNX Runtime for mobile deployment (Android + iOS)
- Flutter ONNX plugin (`onnxruntime_flutter` or `flutter_onnxruntime`)
- Mandarin G2P lexicon (CC-CEDICT based)

### 1.6 Model Export Strategy

**Single format for all platforms:** PyTorch → ONNX → ONNX Runtime Mobile

| Platform | Runtime | Acceleration |
|----------|---------|--------------|
| Android | ONNX Runtime Mobile | NNAPI |
| iOS | ONNX Runtime Mobile | CoreML EP |
| Python (dev) | ONNX Runtime | CPU/CUDA |

**Advantages over TFLite + CoreML:**
- Single model file for all platforms
- Direct PyTorch → ONNX export (no TensorFlow conversion)
- Consistent behavior across platforms
- Easier debugging (same model everywhere)

### 1.7 Data Strategy

#### Data Sources

| Source | Purpose | Size | Format |
|--------|---------|------|--------|
| **App TTS audio** | Tone templates, reference contours, pipeline testing | ~200 sentences × 2 voices | MP3 → WAV |
| **User recordings** | Calibration, real-world testing | Collected via app | M4A → WAV |
| **AISHELL-1** | CTC model training (optional) | 170 hours, 400 speakers | WAV + transcripts |
| **AISHELL-3** | Multi-speaker TTS training (optional) | 85 hours, 218 speakers | WAV + transcripts |

#### Bootstrap Strategy (No External Datasets)

For initial development, use app TTS data + user recordings only:

1. **Tone templates:** Extract contours from TTS audio (known-correct tones)
2. **Alignment:** Use pre-trained forced aligner (Montreal Forced Aligner / WeNet)
3. **Tone classifier:** Train on TTS contours (2 voices × 200 sentences = 400 samples per tone)
4. **Calibration:** Collect user recordings via app, manually label subset

#### Dataloader Specification

```python
# tools/mandarin_grader/mandarin_grader/data/dataloader.py
...
class AudioSample:
    ...
    audio_path: Path
    text: str
    romanization: str  # "nǐ hǎo"
    syllables: list[TargetSyllable]  # Parsed from romanization
    source: Literal["tts", "user", "aishell"]
...
class SentenceDataset:
    ...
    @classmethod
    def from_aishell(cls, aishell_root: Path) -> "SentenceDataset":
...
class ContourDataset:
    ...
    def __init__(self, samples: list[AudioSample], aligner: ForcedAligner):
        self.samples = samples
        self.aligner = aligner

    def extract_all(self) -> list[tuple[Contour, Tone]]:
        """Extract contours from all samples using forced alignment."""
        ...

    def to_pytorch(self) -> "torch.utils.data.Dataset":
...
```
MP3/M4A → WAV (16kHz mono) → Mel Spectrogram → Model
                          ↓
                     F0 Extraction → Contours → Tone Classifier
```
...
def extract_mel(audio: np.ndarray, sr: int = 16000) -> np.ndarray:
    """Extract 80-dim log-mel spectrogram."""
    import librosa
    mel = librosa.feature.melspectrogram(
        y=audio, sr=sr, n_mels=80,
        hop_length=160, win_length=400  # 10ms hop, 25ms window
    )
...
2. **Trained tone classifier** (ONNX format)
...
- Deploy ONNX Runtime models on Android and iOS
...
Flutter (Dart)
    |
    v
MandarinScorerPlugin (flutter_onnxruntime or onnxruntime_flutter)
    |
...
**Flutter ONNX Package Options:**
- `flutter_onnxruntime`: Lightweight, auto-fetches official libraries
- `onnxruntime_flutter`: FFI-based, good performance
- `onnxruntime_flutter_pro`: GPU support (Qualcomm, CUDA)

### 6.3 ONNX Runtime Integration (Android + iOS)

**Model Files (shared across platforms):**

| Model | File | Size | Purpose |
|-------|------|------|---------|
| Alignment | ctc_aligner_int8.onnx | 5 MB | Forced alignment |
| Tone | tone_clf_int8.onnx | 3 KB | Tone classification |

**Model Export from PyTorch:**

```python
# Export alignment model
torch.onnx.export(
    model,
    dummy_input,
    "ctc_aligner.onnx",
    input_names=["mel_spectrogram"],
    output_names=["posteriors"],
...
quantize_dynamic("ctc_aligner.onnx", "ctc_aligner_int8.onnx")
```

**Dart Interface (using flutter_onnxruntime):**

```dart
import 'package:flutter_onnxruntime/flutter_onnxruntime.dart';

class MandarinScorerOnnx {
  late OrtSession _alignmentSession;
  late OrtSession _toneSession;
  ...
  Future<void> initialize(String modelsPath) async {
    ...
    // Use platform-specific acceleration
    if (Platform.isAndroid) {
      options.appendNnapiExecutionProvider();
    } else if (Platform.isIOS) {
      options.appendCoreMLExecutionProvider();
    }

    _alignmentSession = await OrtSession.fromAsset(
      '$modelsPath/ctc_aligner_int8.onnx',
      options: options,
    );
    _toneSession = await OrtSession.fromAsset(
      '$modelsPath/tone_clf_int8.onnx',
      options: options,
    );
  }

  Future<ScoringResult> score(Float32List audio, List<TargetSyllable> targets) async {
    // 1. Extract mel spectrogram
    final mel = extractMelSpectrogram(audio);

    // 2. Run alignment model
    final posteriors = await _alignmentSession.run({'mel_spectrogram': mel});
...
2. **NNAPI EP:** Use ONNX Runtime NNAPI execution provider on Android
3. **CoreML EP:** Use ONNX Runtime CoreML execution provider on iOS
...
6. **Final model artifacts** (ONNX, quantized INT8)
...
|-----------|-------------|--------|-------|
| CTC Alignment Model | 5 MB | INT8 ONNX | Quantized from 20 MB FP32 |
| Tone Classifier | 3 KB | INT8 ONNX | Tiny MLP |
