#!/usr/bin/env python3
"""Export SyllablePredictorV6 to ONNX format.

This script exports a trained V6 PyTorch checkpoint to ONNX format.
V6 uses sliding window attention with global attention on the last 2 tokens.

Key V6 Architecture:
- CNN downsampling: 4x (2 Conv1d layers with stride=2)
- Sequence structure: [audio | BOS | pos_token] - position tokens at END
- Attention: Sliding window + global attention on LAST 2 tokens (BOS + position)
- RoPE cache: Sized for max_audio_frames // 4 + max_positions + 2

Usage:
    # Basic export
    python export_onnx_v6.py --checkpoint checkpoints_v6/best_model.pt --output model_v6.onnx

    # Export with validation
    python export_onnx_v6.py --checkpoint checkpoints_v6/best_model.pt --output model_v6.onnx --validate

    # Export with metadata generation
    python export_onnx_v6.py --checkpoint checkpoints_v6/best_model.pt --output model_v6.onnx --metadata

Model Information:
    - Input: mel [batch, 80, time], position [batch, 1], audio_mask [batch, time]
    - Output: syllable_logits [batch, 532], tone_logits [batch, 5]
    - Architecture: CNN (4x downsampling) + Position Tokens + RoPE Transformer + Sliding Window Attention + PMA Pooling
"""
...
def infer_config_from_state_dict(state_dict: dict) -> dict:
    ...
    V6-specific inference:
    - d_model: from audio_type_embed shape
    - n_layers: from transformer_layers.* keys
    - n_heads, dim_feedforward: from attention/FFN weights
    - max_positions: from position_embed.weight shape
    - CNN downsampling: 4x (2 Conv1d layers)
    - max_audio_frames: from RoPE cache size: (rope_seq_len - max_positions - 2) * 4
    """
    params = {}

    # Infer d_model from audio_type_embed
    ...
    if "position_embed.weight" in state_dict:
        ...
        params["max_positions"] = 60  # V6 default

    # V6 CNN downsampling factor: 2 Conv1d layers with stride=2 = 4x downsampling
    # Count Conv1d layers in audio_cnn to verify
    ...
    # V6 has 2 Conv1d layers = 4x downsampling (default)
    num_cnn_layers = len(cnn_layer_indices) if cnn_layer_indices else 2
    params["cnn_downsampling_factor"] = 2 ** num_cnn_layers

    # Infer max_audio_frames from RoPE cache
    # V6 RoPE cache size = max_audio_frames // 4 + max_positions + 2
    if "rope.cos_cached" in state_dict:
        rope_seq_len = state_dict["rope.cos_cached"].shape[1]
        # Reverse the formula: max_audio_frames = (rope_seq_len - max_positions - 2) * 4
        params["max_audio_frames"] = (rope_seq_len - params["max_positions"] - 2) * params["cnn_downsampling_factor"]
    else:
        params["max_audio_frames"] = 1000  # Default

    # Attention window cannot be inferred from state_dict (stored as scalar attribute)
    params["attention_window"] = 32

    # Global attention flag - default for V6
    params["use_global_attention"] = True

    return params


def create_model_for_export(checkpoint_path: Path, device: str) -> tuple[SyllablePredictorV6, SyllablePredictorConfigV6, dict]:
    ...
    print(f"  cnn_downsampling_factor: {inferred.get('cnn_downsampling_factor', 4)}x")
...
def prepare_dummy_inputs(config: SyllablePredictorConfigV6, device: str) -> tuple:
    """Prepare dummy inputs for ONNX export.

    V6 inputs:
    - mel: [batch, n_mels, max_audio_frames]
    - position: [batch, 1] (single integer)
    - audio_mask: [batch, max_audio_frames] (bool)
    """
    # Full audio up to max_audio_frames
...
def export_to_onnx(
    ...
    The sliding window attention mask is created dynamically in the model's forward()
    based on config.attention_window. During ONNX export, this mask is baked into the
    graph for the max_audio_frames sequence length.
    """
    ...
    # V6 inputs: mel, position, audio_mask
    ...
    print(f"  Note: Sliding window attention mask is baked for max sequence length")
...
def generate_model_metadata(
    ...
    """Generate model metadata JSON for V6."""
    model_size_bytes = onnx_path.stat().st_size
    model_size_mb = model_size_bytes / (1024 * 1024)

    # Get downsampling factor from inferred params or default (4 for V6)
    ...
    metadata = {
        ...
        "model_architecture": {
            ...
            "attention_notes": "Local sliding window (size=2*window+1) with global attention on last 2 tokens (BOS + position)",
        }
    }
...
def save_model_metadata(metadata: dict, onnx_path: Path) -> Path:
    ...
    print(f"  Model version: V6 (full-sentence, position-based, sliding window attention)")

    return metadata_path


def validate_onnx_export(
    pytorch_model: SyllablePredictorV6,
    onnx_path: Path,
    config: SyllablePredictorConfigV6,
    ...
    # ONNX export uses fixed input size due to attention mask being baked in
...
def main():
    parser = argparse.ArgumentParser(description="Export SyllablePredictorV6 to ONNX")
    ...
    print("=" * 60)
    print("SyllablePredictorV6 ONNX Export")
    print("=" * 60)
