#!/usr/bin/env python3
"""Training script for SyllablePredictorV5.

Trains the full-sentence syllable+tone model V5 with:
- Full sentence audio (up to 10s)
- Position embedding for syllable selection
- Non-autoregressive prediction

Usage:
    # Run overfit test first
    python train_v5.py --overfit-test

    # Full training with AISHELL-3 data
    python train_v5.py --data-source aishell3 --epochs 30

    # Resume from checkpoint
    python train_v5.py --epochs 50 --checkpoint-dir checkpoints_v5 --resume best_model.pt
"""
...
DEFAULT_CHECKPOINT_DIR = Path(__file__).parent.parent / "checkpoints_v5"
...
def create_dataloader(
    ...
    """Create PyTorch DataLoader for V5 training."""
    import torch
    ...
    from mandarin_grader.model.syllable_predictor_v5 import (
        SyllablePredictorConfigV5, SyllableVocab,
    )
    ...
    # Create V4 config for mel extraction (same parameters)
    ...
    def collate_fn(batch):
        ...
        return {
            ...
            "position": torch.tensor(positions, dtype=torch.long).unsqueeze(1),  # [batch, 1]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train SyllablePredictorV5")
    ...
    logger.info("=" * 60)
    logger.info("SyllablePredictorV5 Training")
    logger.info("=" * 60)
    ...
    logger.info(f"Max audio duration: {args.max_duration_s}s")
    ...
    # Create V5 model
    from mandarin_grader.model.syllable_predictor_v5 import SyllablePredictorV5, SyllablePredictorConfigV5
    model_config = SyllablePredictorConfigV5(
