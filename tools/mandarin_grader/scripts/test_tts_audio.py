#!/usr/bin/env python3
"""Test TTS audio files with the V4 model to verify model works."""
import json
import sys
import io
...
TTS_DIR = Path(__file__).parent.parent.parent.parent / "apps/mobile_flutter/assets/examples"
...
def main():
    print("=" * 60)
    print("Testing TTS Audio with V4 Model")
    print("=" * 60)
    ...
    # Test first 5 sentences with female TTS
    test_ids = ["ts_000001", "ts_000002", "ts_000003", "ts_000006", "ts_000009"]

    for ts_id in test_ids:
        audio_path = TTS_DIR / "female" / f"{ts_id}.mp3"
        if not audio_path.exists():
            print(f"Skipping {ts_id} - file not found")
            continue
